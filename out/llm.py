    ...
    data_dict = {
        ...
        "技术指标": {
            date: {
                ...
                "布林带": {
                    "上轨": f"{technical_indicators_dict.loc[date, 'BOLL_UPPER']:.2f}",
                    "中轨": f"{technical_indicators_dict.loc[date, 'BOLL_MID']:.2f}",
                    "下轨": f"{technical_indicators_dict.loc[date, 'BOLL_LOWER']:.2f}"
                },
                "动向指标": {
...
class LLMAnalyzer:
    ...
    def request_analysis(self, df: pd.DataFrame, technical_indicators: pd.DataFrame, stock_code: str = None, stock_name: str = None) -> Optional[Dict[str, Any]]:
        """
        向 llm API 发送分析请求
        ...
        try:
            ...
            user_message = f"请分析以下股票数据并给出专业的分析意见：\n\n{stock_info}{data_str}"
            ...
            # 保存原始输出内容到结果中，供外部保存到文件
            result['_raw_analysis_text'] = analysis_text
